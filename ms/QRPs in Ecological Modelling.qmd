...
QRP_tbl <- readr::read_csv(here::here("data", "tbl-QRP-frequency.csv"),
  ...
  gt::gt(groupname_col = "Category",
         ...
         caption = 'Examples and self-reported frequency of questionable research practices in hypothesis-testing research in Ecology and Evolutionary Biology. QRPs are categorised as "cherry-picking", "p-hacking", and "methodologically flawed", indicated by the cherry, saw, and cross icons respectively.') %>% 
...
### Common QRPs associated with Null Hypothesis Significance Testing in Ecology
...
#| tbl-cap: "The questionable practice may target different components of a publishable model (table columns) and it may alter either the model components directly, or indirectly (rows). Note that the colours red and green correspond to the model itself, (Red: $M_p$ ) or model outputs(Green: $M_j$).The final row represents the unambiguous ideal where no QRP occurs— analytical decisions are pre-specified."
tibble::tibble(QRP_outcome = c("The Model or Model Outputs<br>$(M, M_j)$", "Model Credibility or Utility, via selective reporting or 'fishing'", "No QRP, e.g. pre-specified analytic decisions"),
               QRP_target_M = c("$$C (y, M;\\phi (y,M))$$", 
                                "$$C (y, M;{\\phi}^{best} (y,M)) $$", 
                                "$$C (y, M;\\phi ) $$"),
               QRP_target_Mj = c("$$ C (y, M;{\\phi}_{{M}_{j}} (y,M))$$",
                                 "$$C (y, M;{{\\phi}_{{M}_{j}}}^{best} (y,M))$$",
                                 "$$ C (y, M;{\\phi}_{{M}_{j}})$$"),
               row_group = c("QRP", "QRP", "OK")) %>% 
  gt::gt(
    # caption = gt::md("The questionable practice may target different components of a publishable model (table columns) and it may alter either the model components directly, or indirectly (rows). Note that the colours red and green correspond to the model itself, (Red: $M_p$ ) or model outputs(Green: $M_j$).The final row represents the unambiguous ideal where no QRP occurs— analytical decisions are pre-specified.")
) %>% 
