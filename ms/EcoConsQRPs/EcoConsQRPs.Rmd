...
- facet by method
- Group by method, then QRP and compute proportion responding "Yes" (y-axis)
- QRP_key (x-axis)
- colour fill by expert vs. non-expert method
...
dat %>% dplyr::group_by(method, QRP_key, expert_status, self_report) %>% 
        ...
        dplyr::mutate(total = sum(n), proportion_self_report = n/total,
                      lower = binom.confint(n, total, method=c("wilson"),type="central")$lower,
                      upper = binom.confint(n, total, method=c("wilson"),type="central")$upper) %>%
