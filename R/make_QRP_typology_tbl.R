...
typology <- tibble(
        ...
        Prespec_Reporting = c(
                ...
                "Data-dependent and defensible modelling decisions $C(\\boldsymbol{X}, T;\\phi_D)$, with pre-specified reporting choices $S_{\\text{pre}}$.",
        ...
        Selective_Reporting = c(
                "Selective Reporting:<br>$S_Q\\{C(X,M,\\phi\\})$",
                "$S_Q\\{C(X,M,\\phi(M_j))\\})$",
                ...
                "Selective Reporting:<br>$S_Q(\\{C(X,M;\\phi_D\\})$",
                "$S_Q(\\{C(X,M;\\phi_D(M_j))\\})$",
...
typology_tbl <- typology %>%
        ...
        gt::tab_style(style = cell_borders(style = "hidden",sides = "bottom"), 
                      locations = cells_body(rows = c(1,2,4,5,7,8), 
                                             columns = everything())) %>% 
        gt::tab_style(style = cell_text(v_align = "top"),
                      locations = cells_body(columns = everything(), 
                                             rows = c(3,6)))

gt::gtsave(typology_tbl, path = here::here("tables/"), filename = "QRP_typology.png")
